...
namespace MyFirstCSharp
{
    public partial class Final_Exam03 : Form
    {
        ...
        *@explain : 고객이 주문한 개수
        **/
        public int appleCountOrder = 0; // 고객이 주문한 사과 개수
        ...
        **/
        ...
        {
            if (appleCountOrderTemp == 0 && melonCountOrderTemp == 0 && watermelonCountOrderTemp == 0)
            {
                MessageBox.Show("담긴 상품이 없습니다.");
                return;
            }
            ...
            // 재고 수량 복구
            lblAppCount.Text = (Convert.ToInt32(lblAppCount.Text) + appleCountOrderTemp).ToString();
            lblMelonCount.Text = (Convert.ToInt32(lblMelonCount.Text) + melonCountOrderTemp).ToString();
            lblW_MCount.Text = (Convert.ToInt32(lblW_MCount.Text) + watermelonCountOrderTemp).ToString();
        ...
        **/
        ...
        /**
        *@explain : 개별 마진 보기
        *error: 마진 버튼을 누르기만해도 누적값이 계속 증가함....
        **/



        private int appleMarginTotal = 0; // 사과 마진 누적 변수
        private int melonMarginTotal = 0; // 참외 마진 누적 변수
        private int watermelonMarginTotal = 0; // 수박 마진 누적 변수
        ...
        private void btnShowUnitMargin_Click(object sender, EventArgs e)
        {
            ...
            int.TryParse(txtAppleInvoieCount.Text, out int _appleCount);
            int.TryParse(txtMelonInvoieCount.Text, out int _melonCount);
            int.TryParse(txtW_MInvoieCount.Text, out int _watermelonCount);
            ...
            if (rdoApple.Checked)
            {
                ...
                int appleMargin = 2000 * appleCount-(1200*_appleCount); // 사과 주문에 대한 마진 계산
                appleMarginTotal += appleMargin; // 사과 마진 누적
            ...
            else if (rdoMelon.Checked)
            {
                ...
                int melonMargin = 2500 * melonCount - (1500 * _melonCount); // 참외 주문에 대한 마진 계산
                melonMarginTotal += melonMargin; // 참외 마진 누적
            ...
            else if (rdoW_M.Checked)
            {
                ...
                int watermelonMargin = 18000 * watermelonCount - (10800 * _watermelonCount); // 수박 주문에 대한 마진 계산
                watermelonMarginTotal += watermelonMargin; // 수박 마진 누적
